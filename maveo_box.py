...
class MaveoBox:
    ...
    async def init_connection(self) -> str | None:
        ...
        try:
            ...
            handshake_message = self._send_raw("JSONRPC.Hello", {})
            self._use_ssl = False
        except Exception:
    ...
    def _reconnect(self) -> None:
        """Reconnect the JSON-RPC socket after a connection loss."""
        _LOGGER.warning("Reconnecting to maveo box at %s:%s", self._host, self._port)
        try:
            self._sock.close()
        except Exception:
            pass
        ...
        # Re-do handshake
        self._send_raw("JSONRPC.Hello", {})
        _LOGGER.info("Reconnected to maveo box successfully")
    ...
    def _send_raw(
        ...
        """Send a raw command via JSON-RPC socket without reconnection logic."""
    ...
    def send_command(
        ...
        """Send a command via JSON-RPC socket and wait for response, with reconnection."""
        with mutex:
            try:
                return self._send_raw(method, params)
            except (ConnectionResetError, RuntimeError, OSError) as ex:
                _LOGGER.warning("Connection lost, attempting reconnect: %s", ex)
                try:
                    self._reconnect()
                    return self._send_raw(method, params)
                except Exception as reconnect_ex:
                    _LOGGER.error("Reconnection failed: %s", reconnect_ex)
                    return None
